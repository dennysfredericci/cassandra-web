...
func (h *Handler) Export(c echo.Context) error {
	...
	cmdStr := fmt.Sprintf("HOST=%s KEYSPACE=%s TABLE=%s DIRECTORY=%s cexport", strings.Split(env.CassandraHost, ",")[0], strings.Split(table, ".")[0], strings.Split(table, ".")[1], tmpPath)
